...
-- index on catalog sales
CREATE INDEX if not exists idx_cs_ext_list_price on catalog_sales(cs_ext_list_price);
CREATE INDEX if not exists idx_cs_ext_wholesale_cost on catalog_sales(cs_ext_wholesale_cost);
CREATE INDEX if not exists idx_cs_ext_discount_amt on catalog_sales(cs_ext_discount_amt);
CREATE INDEX if not exists idx_cs_ext_sales_price on catalog_sales(cs_ext_sales_price);
-- index on web sales
...
catalog_total as(
 select c_customer_id customer_id
       ...
       ,'c' sale_type
 from customer
     inner join catalog_sales on c_customer_sk = cs_bill_customer_sk
...
inner join t_c_firstyear on t_s_firstyear.customer_id = t_c_firstyear.customer_id
inner join t_c_secyear on t_s_firstyear.customer_id = t_c_secyear.customer_id
